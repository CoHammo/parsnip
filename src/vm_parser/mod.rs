...
impl<T: Parses> Parser<T> {
    pub fn new(ops: Vec<Op<T>>) -> Self {
        Self {
            stat: Stat::Running,
            debug: false,
            // ops: Ops::new(ops),
    ...
    pub fn take_snip<I: SnipIter<T>>(&mut self, snip: &Snip<T>) {
            ...
            && self.stat == Stat::Running
        {
            ...
            let ptr = self as *mut Parser<T>;
            while self.fops.call(ptr, snip, id) {}
            // loop {
            //     if self.debug {
            //         println!(
            ...
            //             let thread = &mut self.threads[id];
            //             ip += 1;
            //             thread.stack = self.stack.push_stack(Var::loo(ip), thread.stack);
            //         }
            //         END_LOOP => {
            //             let thread = &mut self.threads[id];
